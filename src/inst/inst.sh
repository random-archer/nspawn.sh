...
# make package content digest
ns_inst_hash() {
    local hash=;
    hash=$(md5sum "$BASH_SOURCE")
    hash=($hash) # split in array
    echo "$hash" # use first entry
}
...
ns_inst_path() {
    ...
    local inst_hash=; inst_hash=$(ns_inst_hash)
    local exec_dir="$inst_dir/$ns_build_name/$inst_hash" # program folder
